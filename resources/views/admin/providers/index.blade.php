        ...
        <div class="box box-block bg-white">
            ...
            <table class="table table-striped table-bordered dataTable" id="table-2">
                ...
                @foreach($providers as $index => $provider)
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <!-- <td>{{ substr($provider->first_name, 0, 1).str_repeat('*', strlen($provider->first_name)) }} {{ substr($provider->last_name, 0, 1).str_repeat('*', strlen($provider->last_name)) }}</td>
                        @if(Setting::get('demo_mode', 0) == 1)
                        ...
                        <td>+919876543210</td>
                        @else
                        <td>{{ substr($provider->mobile, 0, 3).str_repeat('*', strlen($provider->mobile)) }}</td>
                        @endif -->
